...
class StackelbergGame:
    ...
    def figure_uav_utility(self,uav_utility,num_iters):
        ...
        ax.plot(np.arange(0,num_iters), uav_utility, 
             linewidth=2, label='uav_utility')
        ax.plot(230, round(uav_utility[230],2), marker='x', markersize=5, color='red')
        ax.plot(280, round(uav_utility[280],2), marker='x', markersize=5, color='red')
        ...
        ax.annotate('({},{})'.format(280, round(uav_utility[280],2)), xy=(280, round(uav_utility[280],2)), xytext=(-50, -40), textcoords='offset points',color = 'red', fontweight='bold', arrowprops=dict(arrowstyle='->'))
        ...
        axins.plot(np.arange(0,num_iters), uav_utility, 
             linewidth=1, label='uav_utility')
        ft.zone_and_linked(ax, axins, 150, 250,  np.arange(0,num_iters), [uav_utility], 'bottom')
    ...
    # 服务器能耗图
    def figure_uav_energy_consumption(self,uav_energy_consumption,num_iters):
        plt.plot(np.arange(1,num_iters), uav_energy_consumption,
                     linewidth=1, color='red', label='uav_energy_consumption')
        plt.legend()
        plt.xlabel("number of iterations", fontsize=14)
        plt.ylabel("energy_consumption", fontsize=14)
        plt.xlim(0,300)
        plt.ylim(0)
        plt.show()
...
for i in range(uav_number):
    ...
    for j in range(int(user_number/uav_number)):
        ...
        users.append(User(random.uniform(i * 50 - 25, i * 50 + 25), random.uniform(
            i * 50 - 25, i * 50 + 25), relation=relationship, server=uavs[i],offload=15))
...
print(uav_utility[580:600])
game.figure_uav_utility(uav_utility,len(uavs_indexes3[0]))